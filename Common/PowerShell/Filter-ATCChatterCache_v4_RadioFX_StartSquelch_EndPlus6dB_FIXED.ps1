...
Write-Info "Profile:  Radio filter + end squelch used at start and end" "Cyan"

# Start cue:
#   same soft kshht profile used for the accepted end squelch.
# End squelch:
#   same soft kshht profile used in accepted sample testing.
# Voice filter:
#   same accepted radio/muffle/static filter with 5% volume boost.
$FilterComplex = @"
[1:a]highpass=f=500,lowpass=f=3000,afade=t=in:st=0:d=0.008,afade=t=out:st=0.075:d=0.045,volume=1.24,aformat=sample_rates=48000:channel_layouts=mono[start];
