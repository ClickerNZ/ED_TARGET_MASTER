# v37	-	Add Journal-derived VehicleType for SRV vs Nomad/SLV detection 
#
# ProcessJournal.ps1
#
...
param (
    ...
    [string]$outputFolderPath   = "C:\Thrustmaster\Common\Output",
...
# Module imports
$customModulePath = "C:\Thrustmaster\Common\PowerShell\Modules"
...
# Load lookup maps
Import-MapFile -FilePath "C:\Thrustmaster\Common\PowerShell\Lookup\EDData.json"
...
function Compare-And-UpdateVariables {
    $keys = @(
        ...
        'DeniedReason','LandingPad','Modules','ActiveFighter','VehicleType','InputCMD','CMDParameter',
...
function Send-EDMCOverlayMessage {
    ...
    try {
        ...
        # Silent fail by design: overlay failure must not break journal processing.
    }
}
...
function Process-NewLines {
	...
	#			Write-Host ("->Encoded mask: DEC:{0}  BIN(msb->lsb):{1}" -f $mask, $bin) -ForegroundColor Magenta
			...
			"LaunchSRV" {
				...
				if ($Global:Debug) {
					if (-not $Global:GameRunning) {
						Write-Host "[$localtime] : Event: LaunchSRV, VehicleType = $Global:newVehicleType" -ForegroundColor Yellow 
					}
					else {
						Write-Host "[$localtime] : Event: LaunchSRV, VehicleType = $Global:newVehicleType" -ForegroundColor Cyan
					}
				}																				
			}
			"DockSRV" {
                if (("SRVType" -in $entry.PSObject.Properties.Name) -and ($entry.SRVType -eq "lander01")) {
				...
				if ($Global:Debug) {
					if (-not $Global:GameRunning) {
						Write-Host "[$localtime] : Event: DockSRV, VehicleType = $Global:newVehicleType" -ForegroundColor Yellow 
					}
					else {
						Write-Host "[$localtime] : Event: DockSRV, VehicleType = $Global:newVehicleType" -ForegroundColor Cyan
					}
				}																				
			}
			"Embark" {
				if (("SRV" -in $entry.PSObject.Properties.Name) -and $entry.SRV) {
                    ...
                    elseif (("ID" -in $entry.PSObject.Properties.Name) -and ($entry.ID -eq 66)) {
                        # Fallback from current observed journals where Nomad ShipID/ID is 66.
                        $Global:newVehicleType = "SLV"
                    }
                    elseif (("ID" -in $entry.PSObject.Properties.Name) -and ($entry.ID -eq 67)) {
                        # Fallback from current observed journals where Scarab ShipID/ID is 67.
                        $Global:newVehicleType = "SRV"
                    }
...
# seed new* variables
foreach ($prop in (Get-Variable -Scope Global | Where-Object Name -match '^(LoadGameDetect|CMDRName|ShipName|ShipType|StationName|StationType|SystemName|BodyName|OrganicFound|DockingStatus|DeniedReason|LandingPad|Modules|ActiveFighter|VehicleType|InputCMD|CMDParameter)$')) {	
